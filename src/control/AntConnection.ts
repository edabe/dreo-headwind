...
import { DataHandler, DataType } from '../handler/DataHandler';
import PowerHeartRateMode from '../handler/PowerHeartRateMode';
import AntProfileLogger from '../utils/AntProfileLogger';

/**
 * Extends DataType with ANT specific properties
 */
interface AntDataType extends DataType {
    beatCount: number;
    powerCount: number;
    cadenceCount: number;
}
...
export default class AntConnection {
    ...
    // Ant data logger
    private antLogger: AntProfileLogger;
    ...
    constructor(logger: Logger<ILogObj>, nconf: Provider) {
        ...
        this.dataHandler = new PowerHeartRateMode(logger, nconf);
        this.antLogger = new AntProfileLogger(new Logger<ILogObj>({ 
            name: 'dreo-headwind-ANTlogger',
            minLevel: 3
        }));
    ...
    private async setupSensors(channel: Channel): Promise<void> {
        Object.entries(this.allowedDevices).forEach(async (entry) => {
            const key = entry[0];
    ...
    protected onData(profile: string, deviceId: number, data: SensorState): void {
        ...
        if (this.areDevicesActive()) {
            this.dataHandler.onDataHandler(this.cachedDataType);
        }
    }
    ...
    private async dataHandlerStandBy(): Promise<void> {
        ...
        if (!this.areDevicesActive()) {
            ...
            await this.dataHandler.cleanUp();
        }
    }
     ...
     * 
     * @returns true if and only if all detected ANT devices are active.
     */
    private areDevicesActive(): boolean {
        // Oncly considers PWR device
        return this.activeProfiles.get('PWR') !== undefined;
    }
    ...
    protected async onAppExit(error?: Error) {
        ...
        try {
            ...
            this.logger.info('Cleaning up dataHandler');
            await this.dataHandler.cleanUp();
